    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        {hotel.images && hotel.images.map((img, index) => (
          <img key={index} src={img} alt={`Hotel ${index}`} className="w-full h-48 object-cover rounded" />
        ))}
      </div>
        ...
        <div className="mt-4">
          ...
          <ul className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {hotel.services.map((service, index) => (
              <li key={index} className="bg-gray-100 rounded px-3 py-1 flex items-center gap-2">
                <span>{service}</span>
              </li>
            ))}
          </ul>
      ...
      <div className="mt-6">
        ...
        <p className="mb-2">• يتم الدفع عند الوصول. <br /> • يجب إلغاء الحجز قبل 24 ساعة. <br /> • سياسة الإلغاء قابلة للتغيير حسب تواريخ السفر.</p>
      </div>
